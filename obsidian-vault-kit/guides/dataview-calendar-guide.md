...
### 조건
- 취합 대상 페이지는 frontmatter **`tags`**(Dataview가 태그로 인식하는 예약 필드)에 공통 태그(예: `일정표`) 하나를 추가해서 표시해둬야 함 — 총괄 페이지가 이 태그로 대상을 찾음. `dv.pages("#태그")`는 `tags`/`tag` 예약 필드나 본문 인라인 `#태그`만 인식하므로, 커스텀 이름의 필드(예: 한글 `태그`)에 넣으면 인식되지 않음에 주의
...
// 대상 페이지가 되려면 frontmatter tags 필드에 SOURCE_TAG(아래는 "일정표")를 추가하면 됨.
